function handlePlot=plotSTL(VertexData,FVCD,fignr,holdoff_flag,offset)
...
elseif length(FVCD(:))==3
    ...
    hPlot = patch(VertexData{1},VertexData{2},VertexData{3},zeros(size(VertexData{1})));
    set(hPlot,'facec','flat','EdgeColor',[0 0 0]);
    set(hPlot,'facec',FVCD);
    
else
    
    hPlot = patch(VertexData{1}+offset(1),VertexData{2},VertexData{3}+offset(3),FVCD);
    
%       set(hPlot,'facec','flat','EdgeColor',[0 0 0],'FaceAlpha',1); %grid
    set(hPlot,'facec','flat','EdgeColor','none','FaceAlpha',0.4); %solid 
end
