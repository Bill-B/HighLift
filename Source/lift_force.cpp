...
for (i=0;i<info.nopanel;i++)
{
  ...
  for (j=0;j<info.m;j++)
  {
	 ...
	 {
		//if(j==0) //Cleaned up since we no longer need to average induced velocities at midspan locations. 
		 //now we compute induced velocities at three locations at the leading edge of each DVE //Bill sometime in 2015
		 //removing the averaging seems to work still
		 //We had problems with the averaging with triangles (averaging two points did not leave us at the leading edge)
		//{
...
//#printf("UXS= %lf\n",(UxS));//#
		if (info.analysistype==2||info.analysistype==3){
			if(j==0) 
			{	
				//I think these should be here to capture the trailing edge since we don't have KC until convergence, 
				//but they just made it take longer to converge.
				A = 0;//-1*( surfacePtr[l+panelPtr[i].n*(info.m-1)].A - surfacePtr[l].A - wakePtr[timestep][(i*panelPtr[i].n)+k].A ) ;
				B = 0;//-1*( surfacePtr[l+panelPtr[i].n*(info.m-1)].B - surfacePtr[l].B - wakePtr[timestep][(i*panelPtr[i].n)+k].B ) ;
				C = 0;//-1*( surfacePtr[l+panelPtr[i].n*(info.m-1)].C - surfacePtr[l].C - wakePtr[timestep][(i*panelPtr[i].n)+k].C ) ;
			}
			else
...
//*****************************************************************************
		//if ((j == 0&&info.analysistype==2)||(j == 0&&info.analysistype==3)) N_free = 0; //if this is the first spanwise vortex (actuall
		//the TE bottom
		//else{
...
//#printf("R2     %lf\t%lf\t%lf\n",R2[0],R2[1],R2[2]);//#
		...
		//if ((j ==0&&info.analysistype==2)||(j ==0&&info.analysistype==3)) { //if this is the first spanwise row (actually
			//the TE bottom)
			//R[0] = 0;
			//R[1] = 0;
			//R[2] = 0;
...
//*****************************************************************************
	  	 //Compute total aerodynamic forces of DVE, global ref. frame
//*****************************************************************************
		surfacePtr[l].Force[0] = (N_force[l][4]+N_force[l][5])*eN[0];
		surfacePtr[l].Force[1] = (N_force[l][4]+N_force[l][5])*eN[1];
		surfacePtr[l].Force[2] = (N_force[l][4]+N_force[l][5])*eN[2];

		//introducing density
		scalar(surfacePtr[l].Force,info.density,surfacePtr[l].Force);
