...
//solves equation system for bound circulation distribution
void DVE_Vorticity_Distribution(const GENERAL,const PANEL *,DVE *,\
								DVE **,const int,\
								double **,double *, const int *);
//Assembles top 2/3 of D matrix for DVEs, based on boundary conditions
void DVE_BoundaryCond(const DVE *,const PANEL *,const GENERAL,double **);
//Assembles lower 1/3 of D matrix for DVEs, based on flow tangency condition
...
/*
		 //save D matrix and resultant vector R in file D_matrix.txt
 FILE *fp;
 fp = fopen("D_matrix_rightbefore_solve.txt", "w");
// int n,m;
 //writes header line
 fprintf(fp, "\t");
 for(m=0; m<info.noelement*3; m++)
...
///*
	...
	for(n=0;n<info.nopanel;n++)
	{
		lelement = element;
		//forwards surface-DVE index to trailing egde elements
		element += panelPtr[n].n*(info.m-1);

		//loop over number of spanwise elements of current panel
		for(j=0;j<panelPtr[n].n;j++)
		{

			if (info.analysistype == 0 || info.analysistype ==1){

			wakePtr[timestep][span].A = surfacePtr[element].A;
			wakePtr[timestep][span].B = surfacePtr[element].B;
			wakePtr[timestep][span].C = surfacePtr[element].C;

			//total circulation of DVE, stays constant despite stretching
			wakePtr[timestep][span].K = surfacePtr[element].A \
			...
			else if (info.analysistype == 2||info.analysistype==3){//this doesnt matter because vorticity are recalculated with K.
			wakePtr[timestep][span].A = surfacePtr[element].A-surfacePtr[lelement].A;
			wakePtr[timestep][span].B = surfacePtr[element].B-surfacePtr[lelement].B;
			wakePtr[timestep][span].C = surfacePtr[element].C-surfacePtr[lelement].C;

			//total circulation of DVE, stays constant despite stretching

			wakePtr[timestep][span].K = (surfacePtr[element].A-surfacePtr[lelement].A) \
							+ surfacePtr[element].eta\
							* surfacePtr[element].eta/3\
							* (surfacePtr[element].C-surfacePtr[lelement].C);
	...
	for(m=0;m<info.m;m++)
	{
	  ...
	  {	//compute the velocity induced at suface DVE by wake and free stream
		  ...
		  else
		  {
			  ...
			  Wake_DVE_Vel_Induction\
			  (info,surfacePtr[element].ctrl,wakePtr,timestep,w_wake);  //new with thickness
								//Subroutine in induced_velocity
		  ...
		  R[(element*3)+n2] = 4*Pi*dot(w_extern,X);
		  R[(element*3)+n2+1] = 4*Pi*dot(w_extern,Y);
		  
		  R[(element*3)+n2+2] =  4*Pi*dot(w_extern,Z);
		  }
		...
		//checking if right edge is a free tip
	  	...
	  	{	//compute the velocity ind. at suface DVE by wake and free stream
			...
			else
			{
				...
				Wake_DVE_Vel_Induction\
				(info,surfacePtr[element].ctrl,wakePtr,timestep,w_wake);
									//Subroutine in induced_velocity
...
void DVE_BoundaryCond(const DVE *surfacePtr, const PANEL *panelPtr, \
										const GENERAL info, double **D)
{
//===================================================================//
		//START assembly of Part 1&2 of D-matrix  using DVEs
//===================================================================//
//same as BoundaryCond, but using surfacePtr instead of elementPtr!
//assembles first part of D matrix that is the boundary conditions of
//the elementary wings.  Within a panel, two conditions exists tht are
//continuous vorticity magnitude and slope in spanwise direction.
//Across the panel boundaries the following conditions exist and are
...
// BC = 010 - gamma' = 0
//========================================================================
	int element=0;		//DVE index counter
	int panel;			//panel index
	int n,m;			//spanwise element, chordwise lifting line counter
	int col,row;		//column, row index
	int nextelement;	//index of neighboring element of panel to the right
...
for(panel=0;panel<info.nopanel;panel++)			//loop over panels
	{
		...
		for(m=0;m<info.m;m++)		//loop over chordwise lifting lines
		{
			...
			switch (panelPtr[panel].BC1)	//edge 1 bound. cond.
			{
				case 100: 	//gamma = 0
					D[element][col]   = -1;
					D[element][col+1] = surfacePtr[element].eta;
					D[element][col+2] = -surfacePtr[element].eta*\
										surfacePtr[element].eta;
//#printf("case 100 element %d  nu %lf\n",element,surfacePtr[element].eta);//#
				break;
				...
				case 110: 	//gamma = 0 and gamma' = 0 (free tip)
					...
					D[row][col]   = 0;
					D[row][col+1] = 1;
					D[row][col+2] = -2*surfacePtr[element].eta;
//#printf("case 110 element %d  nu %lf\n",element,surfacePtr[element].eta);//#
				break;
...
//#printf("case 22 element %d  nu %lf\n",element,surfacePtr[element].eta);//#
				...
				case 10:	//gamma' = 0
					...
					D[element][col]   = 0;
					D[element][col+1] = -1;
					D[element][col+2] = 2*surfacePtr[element].eta;
//#printf("case 10 element %d  nu %lf\n",element,surfacePtr[element].eta);//#
				break;
		...
		for(n=1;n<(panelPtr[panel].n);n++) //loop over spanwise elements-1
			{
				...
				//continuous vorticity magnitude
				D[element][col-3] = 1;
				D[element][col-2] = surfacePtr[element-1].eta;
				D[element][col-1] = surfacePtr[element-1].eta*\
									surfacePtr[element-1].eta;
				D[element][col]   = -1;
				D[element][col+1] = surfacePtr[element].eta;
				D[element][col+2] = -surfacePtr[element].eta\
									*surfacePtr[element].eta;
				...
				D[row][col-3] = 0;
				D[row][col-2] = 1;
				D[row][col-1] = 2*surfacePtr[element-1].eta;
		...
		switch (panelPtr[panel].BC2)	//edge 2 bound. cond.
			{
				case 100: 	//gamma = 0
					if(panel<info.nopanel-1) row += info.noelement;
					D[row][col]   = 1;
					D[row][col+1] = surfacePtr[element].eta;
					D[row][col+2] = surfacePtr[element].eta*\
									surfacePtr[element].eta;
				break;
...
//===================================================================//
		//END assembly of Part 1&2 of D-matrix using DVEs
//===================================================================//
...
for(panel=0;panel<info.nopanel;panel++)	//loop over panels
{
  ...
  for(m=0;m<info.m;m++)		//loop over chordwise lift. lines
  {
	...
	for(i=imin;i<imax;i++)	//loop over spanwise elements-1
	{
		...
		row=(element*3)+2*info.noelement;
		}
//##//loop over surface DVE's at whose control points the induced velocity
//##//is being computed.  Defines also row of D-matrix (3i)
//##	for(i=0;i<info.noelement;i++)
...
//##		row=i+2*info.noelement;
		...
		for(j=0;j<info.noelement;j++)
		{
			...
			//computes influence coefficients, a, b, and c, of
			//DVE j on control point on wing surface (with thickness, 02-15-2015 Bill)
			
			DVE_Influence_Coeff\
					(surfacePtr[j],info,surfacePtr[element].ctrl,a,b,c,0);
									//subroutine in induced_velocity.cpp
			...
			//Horstmann Eq.43
			//Using normal to control point on wing surface (for thickness, 02-15-2015 Bill)
			//D[row][col]		= dot(a,surfacePtr[element].cpnorm);
			//D[row][col+1]	= dot(b,surfacePtr[element].cpnorm);
			//D[row][col+2]	= dot(c,surfacePtr[element].cpnorm);
...
///*  ###########################################################
 //save D matrix and resultant vector R in file D_matrix.txt
 FILE *fp;
 fp = fopen("D_matrix_endKindcond.txt", "w");
 int n;
 //writes header line
 fprintf(fp, "\t");
 for(m=0; m<info.noelement*3; m++)
