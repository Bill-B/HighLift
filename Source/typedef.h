...
struct GENERAL
{
	...
	int analysistype;	//0-lifting surface (control points on DVE)
						//1-lifting surface(controlpoints on airfoil surface)
						//2-panelcode(trapezoidal)
						//3-panelcode(triangular)
	int bc;				//0-neumann (flow tangency), 1- dirichlet (zero flow inside)
//dirichlet will cause an over constrained D matrix since we have to have to set u, v and w
//velocities to 0, instead of just the normal velocity as is the case with neumann 
//(flow tangency). I believe the solution to an overconstrained matrix is simply a 
//"curve fit" approach, so the results are questionable. Jan 2016 Bill.
	int relax;			//relaxed wake in time stepping scheme (=1 applied, 0=not)
	int nowing;			//number of separated wings
	int wing1[10],wing2[10];//span index of edges 1 (left) and 2 (right) of wing
	int panel1[10], panel2[10];//indices of panels at left and right edge of wing
						//G.B. 11-5-06
	...
	int wakedebug;		//Turn wake debug on/off (Forces slat wake over main wing. 
						//NOTE! Do a global search for wakedebug)
	double wakeangle;	//Angle of prescribed wake
...
struct PANEL
{
    ...
    int TE1,TE2;		//indices of left and right DVE @ TE of the panel
						//added 8/12/05 G.B.
	int LE1,LE2;		//indices of left and right DVE @ LE of the panel
...
struct DVE
{
	...
	double x1TE[3],x2TE[3]; //points at trailing edge of elements
	...
	double xTE[3],TEvc[3]; //center point at and vector along trailing edge of previous
						//timestep
	double ctrl[3];		//control point when using thickness added 02-15-2015 Bill.
	double cpnorm[3];	//normal to control point (normal to wing surface with thickness) added 02-15-2015 Bill.
};
