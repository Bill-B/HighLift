...
//prescribe the wake
void Prescribe_Wake(const GENERAL,const int,const DVE*,DVE**);
...

{
	...
	int element = 0;		//index to surface elements along trailing edge US
	int lelement = 0;		//index to surface element along lower surf
...
if (info.analysistype == 0 || info.analysistype ==1){
			...
			/*if (lelement == 0) {
				printf("Up A = %f \t", surfacePtr[element].A);
				
				printf("K = %f \t", wakePtr[span].K);}*/
			
}
else if (info.analysistype == 2||info.analysistype==3){//this doesnt matter because vorticity are recalculated with K.
			wakePtr[span].A = surfacePtr[element].A-surfacePtr[lelement].A;
			wakePtr[span].B = surfacePtr[element].B-surfacePtr[lelement].B;
			wakePtr[span].C = surfacePtr[element].C-surfacePtr[lelement].C;

			//total circulation of DVE, stays constant despite stretching

			wakePtr[span].K = (surfacePtr[element].A-surfacePtr[lelement].A) \
							+ surfacePtr[element].eta\
							* surfacePtr[element].eta/3\
			...
			/*if (lelement == 0) {
				printf("Up A = %f \t", surfacePtr[element].A);
				printf("Low A = %f \t", surfacePtr[lelement].A);
				printf("K = %f \t", wakePtr[span].K);}*/
}
...
//&&	for(time=0;time<=rightnow;time++)
	for(time=1;time<=rightnow;time++)	//&&
	{
		...
		{
if(info.wakedebug == 1){ //forcing wake 
			if ((time == rightnow || time == rightnow-1) && span>=20){
				wakePtr[time][span].u[0] = 0;
				wakePtr[time][span].u[1] = 0;
				wakePtr[time][span].u[2] = info.U[0] * sin(info.wakeangle-info.alpha);
			}
			else{
...
if (info.wakedebug == 1){ //forcing wake
	if ((time == rightnow || time == rightnow-1) && wing==2){
			uright[wing][time][0] = 0;
			uright[wing][time][1] = 0;
			uright[wing][time][2] = info.U[0] * sin(info.wakeangle-info.alpha);
	}
	else{
...
//&&	for(time=1;time<rightnow;time++)
	if (rightnow == 1 && info.wakedebug==1){ //forcing wake
			//the zero vector
	tempA[0] = 0; tempA[1] = 0; tempA[2] = 0;
		for(span=20;span<info.nospanelement;span++)
		{
		//post trailing edge
		...
		for(wing=2;wing<info.nowing;wing++)
	{
...
//&	//(time index = rightnow and 0)
	...
	{
		//post trailing edge
		if (span>=20 && info.wakedebug==1){ //forcing wake
			Displace_Point(info,rightnow,\
						wakePtr[rightnow][span].u,\
						tempA,\
						wakePtr[rightnow][span].u,\
	...
	for(wing=0;wing<info.nowing;wing++)
	{
		if (wing == 2 && info.wakedebug==1){ //forcing wake
Displace_Point(info,rightnow,\
						uright[wing][rightnow],\
						tempA,\
						uright[wing][rightnow],\
...
//===================================================================//
//===================================================================//
		//FUNCTION Prescribe_Wake
		//prescribes wake
//===================================================================//
void Prescribe_Wake(const GENERAL info,const int rightnow,\
				const DVE* surfacePtr,DVE** wakePtr)
{
	//relaxing the wake:
...
//*****************************************************************************
//	1. Determines the induced velocity needed to get the angle

//&&	for(time=0;time<=rightnow;time++)
	for(time=1;time<=rightnow;time++)	//&&
	{

		//the velocity needed should be Uinf*tan(the wake angle) 
		for(span=0;span<info.nospanelement;span++)
		{
		wakePtr[time][span].u[0] = 0;
		wakePtr[time][span].u[1] = 0;
		//wakePtr[time][span].u[2] = info.U[0] * tan(info.wakeangle-info.alpha);
		wakePtr[time][span].u[2] = -info.U[2] + info.U[0]*tan(info.wakeangle); //new wake angle, this one seems to be correct
		}
		...
		for(wing=0;wing<info.nowing;wing++)
		{
			...
			Edge_Point(wakePtr[time][span].xo,wakePtr[time][span].nu,\
					   wakePtr[time][span].epsilon,wakePtr[time][span].psi,\
					   wakePtr[time][span].phi0,
					   wakePtr[time][span].eta,\
					   0,xright[wing][time]);
					 					//subroutine in wake_geometry.cpp

			uright[wing][time][0] = 0;
			uright[wing][time][1] = 0;
			//uright[wing][time][2] = info.U[0] * tan(info.wakeangle-info.alpha);
			uright[wing][time][2] = -info.U[2] + info.U[0]*tan(info.wakeangle); //new wake angle, this one seems to be incorrect
			//computing velocity induced at right tip of wing
			//DVE_Induced_Velocity(info,xright[wing][time],surfacePtr,wakePtr,\
								rightnow,uright[wing][time]);
				 						//subroutine in induced_velocity.cpp
