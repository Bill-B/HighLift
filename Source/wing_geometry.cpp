...
void Wing_Generation(const PANEL*,int,int [5],int [5]);
...
//generates new aero-wing model based on geometry generated using FE-model
void FE_Surface_DVE_Generation(const GENERAL,DVE*);
//generates DVE surfaces of multi wing system
void HighLift_Surface_DVE_Generation(const GENERAL,DVE*, PANEL *);\
//===================================================================//
...
void Wing_Generation(const PANEL* panelPtr,const int nopanel,\
					 int wing1[10],int wing2[10],int panel1[10],int panel2[10])
{
	//identifies the separate wings and their span indices of their tips
	//
	//input:
	// 	panel		- information on panelsh
...
//surfacePtr[l].phiLE*RtD,surfacePtr[l].phiTE*RtD,surfacePtr[l].phi0*RtD);
	...
	for (i=0;i<info.noelement;i++)
	{
		...
		//computes point at half chord side of left ledge edge of DVE
		Edge_Point(surfacePtr[i].xo,surfacePtr[i].nu,\
				   surfacePtr[i].epsilon,surfacePtr[i].psi,\
				   surfacePtr[i].phiLE,-surfacePtr[i].eta,\
				   0,surfacePtr[i].xleft);
				 					//subroutine in wake_geometry.cpp
...
{
//moves wing by delta x every time step,
//function updates xo location of surface DVE's and ctrl points (xcp, ycp and zcp)
//
//	input
//	info			general information
//	surfacePtr		surface DVE's
...
double delx[3];
	...
	for(i=0;i<info.noelement;i++)
	{
		...
		//move control point (for thickness, Bill Feb 2015)
		surfacePtr[i].ctrl[0] -= delx[0];
		surfacePtr[i].ctrl[1] -= delx[1];
		surfacePtr[i].ctrl[2] -= delx[2];
	}
}
//===================================================================//
...
//===================================================================//
		//FUNCTION FE_Surface_DVE_Generation
//===================================================================//
void FE_Surface_DVE_Generation\
				(const GENERAL info,DVE* surfacePtr)
{
//Generates Surface DVEs based on a geometry input file.
//function is base on Surface_DVE_Generation.
// A. The coordinates of the four corners, x1LE, x2LE, x1TE, and x2TE, are
//    read in and, then, are used to generate the remaining information of
//    each Surface DVE.
// B. generates the remaining information of the surface DVEs
//
//
//input:
//	info		general information
...
int i,m,n,wing;			//loop counters
int l=0, k;			//l => suface DVE counter,
double singfct;		//decay rate of singularity at edge of vortex sheet
double tempS,tempA[3],tempAA[3];
...
//================================================================================
// A. Read in new coordinates of x1, x2, xo
//================================================================================
	//Reads in coordinates of x1, x2, xo of new geometry determined by FE-model
	Read_New_Geometry(info,surfacePtr); //function in read_input.cpp

//	for(l=0; l<info.noelement; l++)
//	{
//		//read coordinates of x1, x2 and xo
//		printf("%d  %lf  %lf  %lf\n",l,surfacePtr[l].x1[0],surfacePtr[l].x1[1],surfacePtr[l].x1[2]);
//	}


//================================================================================
//DONE  Read in new coordinates of x1, x2, xo
//================================================================================
...
double delPhi,delXSI1[3],delXSI2[3],delX1[3],delX2[3];

	//1. computes new eta nu, epsilon, psi, xsi of DVE
	//   routine similar to the one found in wake_geometry.cpp for relaxing
	for(i=0;i<info.noelement;i++)
	{
	//********************************************************************
	...
	//********************************************************************
		...
		//transform delX2 into local frame
		Glob_Star(delX2,surfacePtr[i].nu,surfacePtr[i].epsilon,\
											surfacePtr[i].psi,delXSI2);
						  				//function in ref_frame_transform.h

		//if everything works => delXSI[2] = 0 and delXSI[1]>0
		if(delXSI2[1] < 0)
		{	printf(" ohwei! around line 765, wing_geometry.cpp\n %2.2lf %2.16lf\n",delXSI2[1],delXSI2[2]);
			exit(0);
		}

		surfacePtr[i].phiLE = atan(delXSI2[0]/delXSI2[1]);	//LE sweep
		surfacePtr[i].eta = 0.5*delXSI2[1];					//halfspan
...
/*		//computing change in sweep
		...
		surfacePtr[i].phiTE += delPhi;
*/
		//transform trailing edge vector into local frame
		Glob_Star(surfacePtr[i].TEvc,surfacePtr[i].nu,surfacePtr[i].epsilon,\
											surfacePtr[i].psi,delXSI2);
						  				//function in ref_frame_transform.h
		//if everything works => delXSI[2] = 0 and delXSI[1] >0
		if(delXSI2[1] < 0)
		...
		surfacePtr[i].phiTE = atan(delXSI2[0]/delXSI2[1]);	//TE sweep
		surfacePtr[i].phi0 = 0.5*(surfacePtr[i].phiTE+surfacePtr[i].phiLE); //midchord sweep
	//********************************************************************
	}
//####################################################################

//2. updates singularity decay factor
	//computing decaying factor for added singularity at wing tip
	k=0;			//k => trailing-edge element counter
...
//3. velocities
	...
	for (i=0;i<info.noelement;i++)
	{
		...
		surfacePtr[i].u[0]=info.U[0];
		surfacePtr[i].u[1]=info.U[1];
		surfacePtr[i].u[2]=info.U[2];
		...
		tempS=1/norm2(surfacePtr[i].u);  //normalized freestream velocity
		surfacePtr[i].U[0]=surfacePtr[i].u[0]*tempS;
		surfacePtr[i].U[1]=surfacePtr[i].u[1]*tempS;
		surfacePtr[i].U[2]=surfacePtr[i].u[2]*tempS;
	}
}
//===================================================================//
		//END FUNCTION FE_Surface_DVE_Generation
//===================================================================//

//===================================================================//
		//FUNCTION HighLift_Surface_DVE_Generation
//===================================================================//
void HighLift_Surface_DVE_Generation\
				(const GENERAL info,DVE* surfacePtr, PANEL *panelPtr)
{
//Generates Surface DVEs based on a geometry input file.
//function is base on Surface_DVE_Generation.
// A. The coordinates of the four corners, x1LE, x2LE, x1TE, and x2TE, are
...
//================================================================================
	//Reads in coordinates of x1, x2, xo of new geometry determined by FE-model
	//reads in geometry of multiwings
    Read_Multi_Wings(info,surfacePtr,panelPtr);	//Subroutine in read_input.cpp
...
double delPhi,delXSI1[3],delXSI2[3],delX1[3],delX2[3];
	...
	//********************************************************************
		...
		//computing vector from X1LE to X2TE
		delX1[0] = surfacePtr[i].x2TE[0] - surfacePtr[i].x1[0];
		delX1[1] = surfacePtr[i].x2TE[1] - surfacePtr[i].x1[1];
		delX1[2] = surfacePtr[i].x2TE[2] - surfacePtr[i].x1[2];
		//vector from X2LE to X1TE
		delX2[0] = surfacePtr[i].x1TE[0] - surfacePtr[i].x2[0];
		delX2[1] = surfacePtr[i].x1TE[1] - surfacePtr[i].x2[1];
		delX2[2] = surfacePtr[i].x1TE[2] - surfacePtr[i].x2[2];

		//computing the normal of DVE,
		cross(delX2,delX1,tempA);
		scalar(tempA,1/norm2(tempA),surfacePtr[i].normal);

		//recomputing vector from center of leading edge to reference point
		delX1[0] = surfacePtr[i].xo[0] \
					- 0.5*(surfacePtr[i].x1[0]+surfacePtr[i].x2[0]);
		delX1[1] = surfacePtr[i].xo[1] \
					- 0.5*(surfacePtr[i].x1[1]+surfacePtr[i].x2[1]);
		...
		//recompute vector along leading edge of DVE
		delX2[0] = surfacePtr[i].x2[0] - surfacePtr[i].x1[0];
		delX2[1] = surfacePtr[i].x2[1] - surfacePtr[i].x1[1];
		delX2[2] = surfacePtr[i].x2[2] - surfacePtr[i].x1[2];
if (info.analysistype == 0|| info.analysistype ==1){
	//recomputing the normal of DVE if not using panelcode,
		cross(delX1,delX2,tempA);
		scalar(tempA,1/norm2(tempA),surfacePtr[i].normal);
}

//why do we do this twice?? bill 
...
//===================================================================//
		//END FUNCTION HighLift_Surface_DVE_Generation
//===================================================================//
