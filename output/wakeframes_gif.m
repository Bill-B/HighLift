...
for step = 1:laststep
    ...
    [output,cmdout]=system('Main_FindDVEs.exe < temp.txt');
    ...
    patch(wing(:,1:4)',wing(:,5:8)',wing(:,9:12)','-k','FaceAlpha',0.6,'EdgeColor','k');
    if strcmp(sym,'y')==1
        patch(wing(:,1:4)',-wing(:,5:8)',wing(:,9:12)','-k','FaceAlpha',0.6,'EdgeColor','k');
    end
    hold off
    ...
    patch(data(:,2:5)',data(:,6:9)',data(:,10:13)',(data(:,1))','FaceAlpha',0.5,'EdgeColor','k')
    if strcmp(sym,'y')==1
        patch(data(:,2:5)',-data(:,6:9)',data(:,10:13)',(data(:,1))','FaceAlpha',0.5,'EdgeColor','k')
    end
